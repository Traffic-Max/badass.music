...
music_folder = '/Users/pwn.show/projects_garbage/badass_music_channel/music_preprocessor/preprocessed_data'

photos_folder = '/Users/pwn.show/projects_garbage/badass_music_channel/trash/photos'
...
music_descriptor = AI_Music_Generator()
...
async def read_treack_metadata(message: types.Message, file_path='/Users/pwn.show/projects_garbage/badass_music_channel/music_moderator/SOLOMUN - KING OF THE AFTERPARTY MIX (TULUM 2023).mp3'):
    tag = TinyTag.get(file_path)
    duration_data = str(tag.duration).split('.')[0]
    metadata = (
        f"Артист: {tag.artist}\n"
        f"Альбом: {tag.album}\n"
        f"Название трека: {tag.title}\n"
        ...
        f"Год: {tag.year}\n"
        f"Жанр: {tag.genre}\n"
        f"Длительность (сек): {duration_data}\n"
        f"Битрейт (кбит/с): {tag.bitrate}\n"
        f"Количество каналов: {tag.channels}\n"
        f"Частота дискретизации (Гц): {tag.samplerate}"
        f"Комментарии: {tag.comment}\n"
    )
    print(metadata)
    return metadata
